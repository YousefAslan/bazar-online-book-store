...
def search(topic):
    ...
    if not cache: 
        for attempt in range(len(catalog_servers)):
            ...
            try:
                responce = requests.get(selectedServer + '/search/' + topic, timeout= (0.3,2))
...
def lookup(id):
    ...
    if not cache: 
        for attempt in range(len(catalog_servers)):
            selectedServer = selectServer(ServerType.CATALOG)
            try:
                responce = requests.get(selectedServer + '/lookup/' + str(id), timeout= (0.3,2))
                if responce.status_code == 200:
...
def update_price(id):
    ...
    for attempt in range(len(catalog_servers)):
        selectedServer = selectServer(ServerType.CATALOG)
        try:
            responce = requests.put(selectedServer + '/update/price/' + str(id), json = jsons, headers = headers, timeout= (0.3,2))
            return jsonify(responce.json()), responce.status_code
        except:
            pass
    return {"message": " server is not ready to handle the request"}, 503
...
def update_item_number(id):
    ...
    for attempt in range(len(catalog_servers)):
        selectedServer = selectServer(ServerType.CATALOG)
        try:
            responce = requests.put(selectedServer + '/update/item/' + str(id), json = jsons, headers = headers, timeout= (0.3,2))
            return jsonify(responce.json()), responce.status_code
        except:
            pass
    return {"message": " server is not ready to handle the request"}, 503
...
def buy(id):
    ...
    for attempt in range(len(catalog_servers)):
        ...
        try:
            responce = requests.put(selectedServer + '/buy/' + str(id), timeout= (0.3,4))
            return jsonify(responce.json()), responce.status_code
        except:
            pass
    return {"message": " server is not ready to handle the request"}, 503
